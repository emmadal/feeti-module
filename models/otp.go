...
type Otp struct {
	...
	Code        string    `json:"code" gorm:"type:varchar(10);not null;index:idx_code" binding:"required,min=5,max=5,numeric"`
	IsUsed      bool      `json:"is_used" gorm:"type:boolean;not null;default:false"`
	PhoneNumber string    `json:"phone_number" gorm:"type:varchar(15);not null;index:idx_phone_number" binding:"required,e164,min=11,max=14"`
	KeyUID      string    `json:"key_uid" gorm:"type:varchar(100);not null;index:idx_key_uid" binding:"required,uuid"`
